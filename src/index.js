...
const assemblyai = async ({
  ...
  try {
    ...
    if (audioUrl) {
      ...
      throw new Error(
        `You need to provide either a filePath or audioUrl to request a transcription. audioUrl takes precedence over filePath if you accidentally provide both.`
      );
    }
    const { id, status, words } = await submit({
      ApiKey,
      upload_url: tmpAudioUrl,
      languageModel,
      acousticModel,
      autoHighlights,
      iabCategories,
